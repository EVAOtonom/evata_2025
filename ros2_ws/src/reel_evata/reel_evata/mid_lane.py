...
class ImageSaver(Node):
    def __init__(self):
        ...
        self.path_pub = self.create_publisher(Path, '/lane_midpoints_path', 10)
...
##############
    ...
    def publish_midpoint_path(self, mid_points):
        if not mid_points or self.latest_pointcloud is None:
            return

        header = self.latest_pointcloud_header
        header.frame_id = "zed_camera_link"  # Aynı frame ile yayın

        path_msg = Path()
        path_msg.header = header

        poses = []
        for (x_pixel, y_pixel) in mid_points:
            # 2D image koordinatlarını 3D pointcloud ile eşle
            pc_height = self.latest_pointcloud.height
            pc_width = self.latest_pointcloud.width

            if x_pixel < 0 or x_pixel >= pc_width or y_pixel < 0 or y_pixel >= pc_height:
                continue

            pc_data = point_cloud2.read_points_numpy(self.latest_pointcloud)
            pc_data = pc_data.reshape((pc_height, pc_width, -1))
            point = pc_data[int(y_pixel), int(x_pixel)]
            x, y, z = point[:3]
            ...
            pose.pose.orientation.w = 1.0  # düz bakıyor gibi kabul et
            poses.append(pose)
    ...
    def detect(self, source, imgsz=640, conf_thres=0.3, iou_thres=0.45, 
        ...
        if len(mid_points) >= 2:
            ...
            self.publish_midpoint_path(mid_points)
